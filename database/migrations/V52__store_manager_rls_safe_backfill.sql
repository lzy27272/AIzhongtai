-- Follow the established V36 tenant-discovery pattern.  The migration owner
-- is deliberately NOBYPASSRLS and tenant itself has FORCE RLS, so V51 could
-- not enumerate tenants on a fresh Flyway connection.  Transactional DDL
-- restores FORCE RLS automatically if any statement below fails.
...
DECLARE
    ...
    discovered_tenant_count INTEGER := 0;
    remaining_count INTEGER;
BEGIN
    FOR tenant_record IN SELECT id FROM tenant ORDER BY id LOOP
        discovered_tenant_count := discovered_tenant_count + 1;
        ...
        -- Fail the migration instead of silently succeeding when a current
        -- hotel manager still lacks the official daily package.
        SELECT count(*)
         ...
         WHERE assignment.tenant_id = tenant_record.id
           ...
           AND NOT EXISTS (
               ...
               WHERE allocation.tenant_id = assignment.tenant_id
                 AND allocation.position_assignment_id = assignment.id
                 AND allocation.target_org_unit_id = assignment.org_unit_id
                 AND allocation.status = 'ACTIVE'
           );

        IF remaining_count > 0 THEN
            RAISE EXCEPTION 'V52 left % current hotel store managers without a daily package in tenant %',
                remaining_count, tenant_record.id;
        END IF;
    END LOOP;

    IF discovered_tenant_count = 0 THEN
        RAISE EXCEPTION 'V52 could not discover any tenant while repairing store-manager assignments';
    END IF;
END $$;
