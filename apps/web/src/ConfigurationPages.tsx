...
export function OrganizationCenter({ identity, permissions, routeParams = {} }: { identity: RoleContext; permissions: string[]; routeParams?: RouteParams }) {
  ...
  const assignmentDraftInvalid = assignmentDraftStarted && (!assignmentForm.orgUnitId || !assignmentForm.positionId || employeeForm.employmentStatus !== 'ACTIVE'
  ...
  const openCreate = (kind: 'org' | 'employee') => {
    ...
    if (kind === 'employee') {
      ...
      setAssignmentForm({ assignmentId: '', employeeId: '', employeeName: '', orgUnitId: '', positionId: '', managerAssignmentId: '', primary: true, assignmentType: 'PERMANENT', responsibleHotelIds: [] })
    }
    setModal(kind)
  }
  ...
  const openEmployeeEdit = (item: Employee) => {
    ...
    setAssignmentForm({ assignmentId: '', employeeId: item.id, employeeName: item.name, orgUnitId: '', positionId: '', managerAssignmentId: '', primary: !data.employees.some((row) => row.id === item.id && row.assignmentId), assignmentType: 'PERMANENT', responsibleHotelIds: [] })
    setModal('employee')
  }
  ...
  return <section className="page-section configuration-page">
    ...
    {modal === 'employee' && <Modal title={editingId ? '编辑员工与任职' : '新建员工与登录账号'} onClose={closeModal} onSave={submit} saving={saving} saveDisabled={Boolean(editingId) && assignmentDraftInvalid} className={editingId ? 'employee-edit-modal' : ''}>
      ...
      {editingId && <>
        <section className="employee-edit-section employee-current-assignments">
          ...
          {employeeAssignments.length === 0 ? <div className="employee-assignment-empty">该员工尚未分配任职</div> : <div className="employee-assignment-list">{employeeAssignments.map((item) => <article key={item.assignmentId}><span><strong>{item.positionName || '未命名岗位'}</strong><small>{item.orgUnitName || '未设置组织'}</small></span><b>{item.primary ? '主岗' : '兼岗'}</b>{item.authorizationScopeType === 'ASSIGNED_HOTELS' && <small>负责门店：{item.responsibleHotelNames || '尚未配置'}</small>}</article>)}</div>}
        </section>
        <section className="employee-edit-section employee-new-assignment">
      ...
      <div className="inline-warning">{editingId ? '保存会同时更新员工资料；如已填写新增任职，则一并创建岗位权限。停用员工会同步停用账号、结束有效任职和角色授权。' : '初始密码仅本次创建时使用，请通过安全渠道交给员工。'}</div>
    </Modal>}
